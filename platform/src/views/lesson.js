...
import { TEACHING_LABEL } from '../content/lessons.js';
...
export function materials(lesson) {
  ...
  return html`<section aria-labelledby="materials-h">
    ...
    ${lesson.materials.map((m) => html`<article class="source" aria-labelledby="src-${m.id}"><h3 id="src-${m.id}">${m.title}</h3>${paras(m.body)}</article>`)}
  </section>`;
}
